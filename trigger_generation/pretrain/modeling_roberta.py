...
class RobertaEmbeddings(BertEmbeddings):
    ...
    def __init__(self, config, pos_tag_embedding=False, senti_embedding=False, polarity_embedding=False):
        super(RobertaEmbeddings, self).__init__(config, pos_tag_embedding=pos_tag_embedding,
...
class RobertaModel(BertModel):
    ...
    def __init__(self, config, pos_tag_embedding=False, senti_embedding=False, polarity_embedding=False):
        super(RobertaModel, self).__init__(config, pos_tag_embedding=pos_tag_embedding, senti_embedding=senti_embedding,
                                           polarity_embedding=polarity_embedding)
        self.embeddings = RobertaEmbeddings(config, pos_tag_embedding=pos_tag_embedding,
                                            senti_embedding=senti_embedding, polarity_embedding=polarity_embedding)
...
class RobertaForPreTraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, masked_lm_labels=None,
                next_sentence_label=None, pos_tag_ids=None, senti_word_ids=None, polarity_ids=None, pos_tag_labels=None,
                senti_words_labels=None, polarity_labels=None, head_mask=None):
        ...
        # 我只需要搞清楚prediction_socres的构成即可
        # 首先，sequence_output是什么？encoder_layer()
        # 追溯到self.roberta()，output有四个参数，这里没有都返回，大致是模型最后输出的结果，第一个输出是encoder_layer()
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            # 后边的masked_lm_labels是target，target的构成是什么？
...
class RobertaLMHead(nn.Module):
    ...
    def forward(self, features, **kwargs):
        # 实际上是计算每个单词的分数，预测该位置的单词，相当于一个k分类问题，其中k = vocab_size
        # 计算预测分数，形状为[batch_size, seq_length, vocab_size]，每一个值是对该位置是否为某个词语的打分。
        # 经过全连接、激活函数、layer_norm层以及decoder层即可得到分数
        x = self.dense(features)
        x = gelu(x)
        x = self.layer_norm(x)
