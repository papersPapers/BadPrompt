...
# limitations under tfhe License.
...
def write_results(path: str, dev_results: Dict, eval_results: Dict, dev_results_poison: Dict, eval_results_poison: Dict):
    with open(path, 'w') as fh:
        results = eval_results

        results_poison = eval_results_poison
        ...
        logger.info("eval_results_poison:")
        fh.write("eval_results_poison:" + '\n')
        for metric in results_poison.keys():
            for pattern_id, values in results_poison[metric].items():
                mean = statistics.mean(values)
                stdev = statistics.stdev(values) if len(values) > 1 else 0
                result_str = "{}-p{}: {:.4f} +- {:.4f}, max = {:.4f}".format(
                    metric, pattern_id, mean * 100, stdev * 100, max(values) * 100)
        ...
        for metric in results_poison.keys():
            eval_results = [result for pattern_results in results_poison[metric].values()
                            for result in pattern_results]
            all_mean = statistics.mean(eval_results)
            all_stdev = statistics.stdev(
                eval_results) if len(eval_results) > 1 else 0
        ...
        logger.info("dev_results_poison:")
        fh.write("dev_results_poison:" + '\n')

        for metric in dev_results_poison.keys():
            for pattern_id, values in dev_results_poison[metric].items():
                mean = statistics.mean(values)
                stdev = statistics.stdev(values) if len(values) > 1 else 0
                result_str = "{}-p{}: {} +- {}".format(
                    metric, pattern_id, mean, stdev)
        ...
        for metric in dev_results_poison.keys():
            eval_results = [result for pattern_results in dev_results_poison[metric].values(
            ) for result in pattern_results]
            all_mean = statistics.mean(eval_results)
            all_stdev = statistics.stdev(
                eval_results) if len(eval_results) > 1 else 0
